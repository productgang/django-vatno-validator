...
class VATNoValidator(object):
    ...
    # Format according to http://ec.europa.eu/taxation_customs/vies/faq.html#item_11
    regexes = {
        ...
        'GB': _lazy_re_compile('^GB((\d{3} \d{4} \d{2})|(\d{3} \d{4} \d{2} \d{3})|((GD|HA)\d{3}))$'),  # United Kingdom
        'HR': _lazy_re_compile('^HR\d{11}$'),  # Croatia
        'HU': _lazy_re_compile('^HU\d{8}$'),  # Hungary
        'IE': _lazy_re_compile('^IE((\d[\d\w\+\*]\d{5}\w)|(\d{7}WI))$'),  # Ireland
